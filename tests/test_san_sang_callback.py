...
def test_toi_dung_app_thi_DU():
    """
    Tên miền CỐ ĐỊNH, gọi tới được, trả đúng app này -> đủ.

    Dùng tên miền riêng chứ không `trycloudflare`: một tunnel TẠM đang sống
    vẫn là tunnel sẽ chết ở lần chạy lại tiếp theo, nên nó có nhánh cảnh báo
    riêng (xem tests/test_tunnel_co_dinh.py). Ca này đo nhánh "tới đúng app",
    nên đầu vào không được lẫn với ca kia.
    """
    co_dinh = "https://api.tenmien.vn/webhook"
    assert san_sang.doc_callback_cong_khai(co_dinh, 200, None, True)["muc"] == san_sang.DU
