...
def test_plugin_row_uu_tien_p_mcp_truoc_p_goi():
    ...
    Phần vẽ dòng đã tách khỏi `loadKyNang` khi màn Kỹ năng gộp lại: thứ tự
    nhánh nay nằm ở `gopKyNang` (chọn nguồn), huy hiệu ở `veDongKyNang`.
    Cùng ràng buộc, hai chỗ đọc.
    """
    gop = _than_ham("gopKyNang")
    assert "p.mcp" in gop, "không phân biệt công cụ của máy chủ MCP"
    # Ba ngôi trong một biểu thức: `mcp` phải đứng trước `goi`, vì công cụ
    # MCP có CẢ HAI cột và cột `goi` của nó là "mcp:<tên máy>".
    assert gop.index("p.mcp") < gop.index("p.goi")

    ve = _than_ham("veDongKyNang")
    assert 'MCP · ${esc(k.mcp' in ve, "mất huy hiệu MCP"
    assert 'gói ${esc(k.goi' in ve, "mất nhãn tên gói"
    # Công cụ của máy chủ MCP và của gói đều KHÔNG có nút Xoá riêng: máy chủ
    # từ chối, và một nút luôn báo lỗi dạy người ta bỏ qua thông báo lỗi.
    # Nút Xoá chỉ vẽ ở nhánh dành riêng cho công cụ TỰ TẠO.
    assert ve.index('nguon === "tu_tao"') < ve.index("data-plugin-xoa")
