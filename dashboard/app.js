...
async function loadKyNang() {
  ...
  $("#kynang-plugin").innerHTML = d.plugin.length
    ? d.plugin.map((p) => `<div class="row">
        ...
        <span class="row__side">
          ${p.mcp
            /* Công cụ đến từ máy chủ MCP mang cùng cột `goi` với công cụ của
             * gói ("mcp:<tên máy>"), nên phải kiểm `p.mcp` TRƯỚC `p.goi` —
             * kiểm ngược lại thì huy hiệu hiện "gói mcp:<tên máy>" trần,
             * đúng chuỗi kỹ thuật mà cột `mcp` sinh ra để tránh phải hiện. */
            ? `<b class="pill">MCP · ${esc(p.mcp)}</b>`
            : p.goi
